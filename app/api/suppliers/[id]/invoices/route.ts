// API Route para obtener facturas de un proveedor específico
...
// GET: Obtener todas las facturas de un proveedor
export async function GET(
  ...
  try {
    ...
    console.log(`📋 [Supplier Invoices] Obteniendo facturas del proveedor: ${id}`);
    ...
    let whereClause = 'WHERE ie.supplier_id = $1';
    let params: any[] = [id];
    let paramIndex = 2;
    ...
    if (search) {
      whereClause += ` AND (ie.invoice_number ILIKE $${paramIndex} OR c.name ILIKE $${paramIndex} OR d.receiver_name ILIKE $${paramIndex})`;
      params.push(`%${search}%`);
      paramIndex++;
    }

    // Consulta principal para obtener facturas del proveedor
    const invoicesQuery = `
      SELECT 
        ie.*,
        d.document_type,
      ...
      LEFT JOIN customers c ON ie.customer_id = c.customer_id
      ${whereClause}
    ...
    const statsQuery = `
      SELECT 
        ...
        COUNT(DISTINCT ie.customer_id) as unique_customers
      FROM invoice_entities ie
      WHERE ie.supplier_id = $1
    `;
    ...
    console.log(`✅ [Supplier Invoices] ${invoices.length} facturas encontradas para proveedor ${id}`);
